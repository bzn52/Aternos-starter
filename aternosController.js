...
class AternosController {
  ...
  async login() {
    try {
      ...
      // Wait for any button or submit element to appear
      await this.page.waitForSelector('button, input[type="submit"]', {
        timeout: 10000,
      });

      // Find and click the login button using JavaScript
      const loginClicked = await this.page.evaluate(() => {
        // Try to find login button by looking for button with "Login" text
  ...
  async startServer() {
    try {
      // Make sure we're on the servers page
      const currentUrl = this.page.url();
      ...
      // If specific server name provided, find it
      if (this.serverName) {
      ...
      const statusInfo = await this.page.evaluate(() => {
        ...
        // Find start button - FIXED: removed invalid :has-text() selector
      ...
      // Server is offline, try to start it
      ...
      await this.page.evaluate(() => {
        // Find start button by text content
        const buttons = Array.from(document.querySelectorAll("button"));
        const startBtn = buttons.find((b) => {
          const text = b.textContent.toLowerCase();
          return text.includes("start") && !text.includes("restart");
  ...
  async getServerStatus() {
    try {
      ...
      // Make sure we're on a server page
      const currentUrl = this.page.url();
      if (!currentUrl.includes("/server")) {
        await this.page.goto("https://aternos.org/servers/", {
          waitUntil: "networkidle0",
